...
async function audit(client, guildId, text) {
  ...
  const db = getDb();
  const settings = await db.prepare('SELECT audit_channel FROM guild_settings WHERE guild_id = ?').get(guildId);
  ...
  try {
    ...
    if (!warned.has(guildId)) {
      ...
      await db.prepare('UPDATE guild_settings SET audit_channel = NULL WHERE guild_id = ?').run(guildId);
    }
  }
}
