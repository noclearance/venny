...
const PROMPTS = {
  ...
  sotw_start: details =>
    `Skill of the Week has begun for **${details.skill || 'a skill'}** (${details.days || 7} days). ${details.wom ? 'Tracked on Wise Old Man.' : 'Discord week only — Wise Old Man is not attached.'}${details.prize ? ` Prize: **${details.prize}**.` : ' Do not invent a prize.'} A trial of dedication.`,
  sotw_end: details =>
    `Skill of the Week for **${details.skill || 'a skill'}** has ended.${details.winner ? ` Champion: **${details.winner}**${details.xp ? ` with ${Number(details.xp).toLocaleString()} XP` : ''}.` : ' Nobody posted gains.'}${details.prize ? ` Prize: **${details.prize}**.` : ' Do not invent a prize.'} ${details.placed || 0} on the board.`,
  ...
  raffle_start: details =>
    `A clan raffle is open.${details.prize ? ` Grand prize: **${details.prize}**.` : ' Staff did not name in-game loot — do not invent a prize.'} ${details.hours ? `Entries close in **${details.hours} hours**, then I draw.` : 'I will draw when time is up.'} Members pay staff in game, link an RSN, then tap Enter Raffle.`,
  raffle_win: details =>
    `A raffle has been drawn. Title: ${details.title || 'the raffle'}.${details.prize ? ` Prize: **${details.prize}**.` : ' Do not invent a prize.'} ${details.entries || 0} entries. Do not invent the winner's name.`,
  raffle_end: details =>
  ...
  botw_start: details =>
    `Boss of the Week is **${details.boss || 'a boss'}** for ${details.days || 7} days. KC from this second counts.${details.prize ? ` Prize: **${details.prize}**.` : ' Do not invent a prize.'}`,
  botw_end: details =>
    `Boss of the Week for **${details.boss || 'the boss'}** has ended.${details.winner ? ` Champion: **${details.winner}**.` : ''}${details.prize ? ` Prize: **${details.prize}**.` : ' Do not invent a prize.'}`,
  vote_sotw: details =>
...
const FALLBACKS = {
  ...
  sotw_start: { title: '{skill} SOTW', description: 'Skill’s locked. Gains from this second count.', color: 12723212 },
  sotw_end: { title: '{skill} SOTW — results', description: 'That’s a wrap. Numbers below — no recounts.', color: 12723212 },
  sotw_standings: { title: 'The Board Stands', description: 'Here is the live Skill of the Week board.', color: 5763719 },
  raffle_start: { title: '{title}', description: 'Pay staff in game, link your RSN, then tap Enter. I’ll draw later.', color: 7162073 },
  raffle_win: { title: '{title} — drawn', description: 'Drawn. That’s the name. Raffle’s dead.', color: 7162073 },
  raffle_end: { title: '{title} — closed', description: 'No winner. The Enter button is dead.', color: 7162073 },
  ...
  botw_start: { title: 'BOTW · {boss}', description: 'KC from this second counts.', color: 10428985 },
  botw_end: { title: 'BOTW · {boss} — results', description: 'Hunt is closed. Board below is final.', color: 10428985 },
...
function venny(job, details = {}, extra = {}) {
...
module.exports = { announce, write, venny, fallbackOf };
